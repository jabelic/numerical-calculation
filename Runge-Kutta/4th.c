...
double runge_kutta(double *t, double *x){
    x[0] = f_true(t[0]);
    printf("Initial: %f\n", f_true(t[0]));
    int i = 0;
    for (i = 1; i <= N; i++){
        double k1 = df(t[i-1], x[i-1]);
        double k2 = df(t[i-1]+DT/2.0, x[i-1]+k1*DT/2.0);
        double k3 = df(t[i-1]+DT/2.0, x[i-1]+k2*DT/2.0);
        double k4 = df(t[i-1]+DT, x[i-1]+k3*DT);
